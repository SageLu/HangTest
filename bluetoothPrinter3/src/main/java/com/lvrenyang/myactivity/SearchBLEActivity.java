...
//sage
public class SearchBLEActivity extends Activity implements OnClickListener {
	...
	private static String TAG = "SearchBLEActivity";
				...
				if (BluetoothDevice.ACTION_FOUND.equals(action)) {
					...
					button.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View arg0) {
							...
							dialog.show();
							WorkService.workThread.connectBle(address);
						}
					});
	...
	static class MHandler extends Handler {
		...
		WeakReference<SearchBLEActivity> mActivity;

		MHandler(SearchBLEActivity activity) {
			mActivity = new WeakReference<SearchBLEActivity>(activity);
		}

		@Override
		public void handleMessage(Message msg) {
			SearchBLEActivity theActivity = mActivity.get();
			switch (msg.what) {
			/**
			 * DrawerService 的 onStartCommand会发送这个消息
			 */

			case Global.MSG_WORKTHREAD_SEND_CONNECTBLERESULT: {
				int result = msg.arg1;
				Toast.makeText(
						theActivity,
